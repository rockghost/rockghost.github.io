...
for old, new in replacements:
    if old in content:
        ...
        short = old[:50]
        if short in content:
            print(f'PARTIAL MATCH for: {short}...', file=sys.stderr)
        else:
            print(f'NOT FOUND: {old[:60]}...', file=sys.stderr)

# Fix corrupted line 256
lines = content.split('\n')
for i, line in enumerate(lines):
    if i >= 254 and i <= 258:
        stripped = line.strip()
        if stripped.startswith('<p>') and '\u0168' in stripped:
            lines[i] = '<p>\ud074\ub77c\uc774\uc5b8\ud2b8 \uce21\uc5d0\uc11c\ub294 \uc774\ub7ec\ud55c \uc54c\ub9bc\uc744 \ucc98\ub9ac\ud558\uae30 \uc704\ud574 \ucf5c\ubc31 \ud568\uc218\ub97c \uc124\uc815\ud574\uc57c \ud569\ub2c8\ub2e4. \uc11c\ubc84\uac00 \uc774\ub7ec\ud55c \uba54\uc2dc\uc9c0\ub97c \ubc1c\uc0dd\uc2dc\ud0a4\uc9c0\ub9cc, \uc0ac\uc6a9\uc790\uc5d0\uac8c \uc5b4\ub5bb\uac8c \ud45c\uc2dc\ud560\uc9c0\ub294 \ud074\ub77c\uc774\uc5b8\ud2b8 \uc560\ud50c\ub9ac\ucf00\uc774\uc158\uc5d0\uc11c \uacb0\uc815\ud569\ub2c8\ub2e4.</p>'
            count += 1
...
print(f'Replaced {count} segments', file=sys.stderr)

# Verify
with open(filepath, 'r', encoding='utf-8') as f:
    text = f.read()

# Check no corrupted chars remain
if '\u0168' in text:
    print('WARNING: corrupted chars still present', file=sys.stderr)
else:
    print('OK: no corrupted chars', file=sys.stderr)

# Check Korean is present
korean_count = sum(1 for c in text if '\uac00' <= c <= '\ud7a3')
print(f'Korean chars found: {korean_count}', file=sys.stderr)
